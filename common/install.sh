...
if $FULL; then
  ...
  if ! $THEM; then
    ui_print " "
    ui_print "   Disabling Google theme..."
    sed -i 's/ro.boot.vendor.overlay.theme/# ro.boot.vendor.overlay.theme/g' $INSTALLER/common/system.prop
    sed -i 's/ro.config.ringtone/# ro.config.ringtone/g' $INSTALLER/common/system.prop
    sed -i 's/ro.config.notification_sound/# ro.config.notification_sound/g' $INSTALLER/common/system.prop
    sed -i 's/ro.config.alarm_alert/# ro.config.alarm_alert/g' $INSTALLER/common/system.prop
    rm -rf $INSTALLER/system/fonts
    rm -rf $INSTALLER/system/media
    rm -rf $INSTALLER/system/vendor/overlay/Pixel
    rm -rf /data/resource-cache
